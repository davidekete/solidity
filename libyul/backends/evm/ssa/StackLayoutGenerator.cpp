...
void StackLayoutGenerator::visitBlock(SSACFG::BlockId const& _blockId)
{
	...
	for (size_t operationIndex = 0; operationIndex < block.operations.size(); ++operationIndex)
	{
		...
		for (StackType::Depth depth{0}; depth < stack.size(); ++depth.value)
			if (stack.slot(depth).isValueID() && !opLiveOutWithoutOutputs.contains(stack.slot(depth).valueID()) && ranges::find(requiredStackTop, stack.slot(depth)) == ranges::end(requiredStackTop))
	...
	if (auto const* cjump = std::get_if<SSACFG::BasicBlock::ConditionalJump>(&block.exit))
	{
		...
		{
			// then we don't have to do anything, the condition is simply consumed by jumpi
		}
		else
		{
